...
class AugmentedQANet(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self._answer_type_predictor = FeedForward(modeling_out_dim + encoding_out_dim,
                                                  ...
                                                  hidden_dims=[modeling_out_dim, 4],
                                                  num_layers=2,
    ...
    def forward(self,  # type: ignore
        ...
        # # Shape: (batch_size, question_length, modeling_dim * 2))
        # question_for_span_start = torch.cat([modeled_question_list[1], modeled_question_list[2]], dim=-1)
        # # Shape: (batch_size, question_length)
        # question_span_start_logits = self._question_span_start_predictor(question_for_span_start).squeeze(-1)
        # # Shape: (batch_size, question_length, modeling_dim * 2)
        # question_for_span_end = torch.cat([modeled_question_list[1], modeled_question_list[3]], dim=-1)
        # # Shape: (batch_size, question_length)
        # question_span_end_logits = self._question_span_end_predictor(question_for_span_end).squeeze(-1)
        # # Shape: (batch_size, question_length)
        ...
        best_combination_log_prob += answer_type_log_probs[:, 2]
        ...
        best_count_log_prob += answer_type_log_probs[:, 3]
        ...
        if answer_as_passage_spans is not None:
            ...
            passage_span_mask = (passage_span_starts != -1).long()
            clamped_passage_span_starts = passage_span_starts * passage_span_mask
            clamped_passage_span_ends = passage_span_ends * passage_span_mask
            # Shape: (batch_size, # of answer spans)
            ...
            log_likelihood_for_passage_spans = \
                util.replace_masked_values(log_likelihood_for_passage_spans, passage_span_mask.float(), -1e10)
            # Shape: (batch_size, )
            ...
            # Some spans are padded with index -1, so we need to mask them
            question_span_mask = (question_span_starts != -1).long()
            clamped_question_span_starts = question_span_starts * question_span_mask
            clamped_question_span_ends = question_span_ends * question_span_mask
            # Shape: (batch_size, # of answer spans)
            ...
            # For those padded spans, we set their log probabilities to be very small negative value
            log_likelihood_for_question_spans = \
                util.replace_masked_values(log_likelihood_for_question_spans, question_span_mask.float(), -1e10)
            # Shape: (batch_size, )
            ...
            log_marginal_likelihood_for_combination = \
                answer_type_log_probs[:, 2] + util.logsumexp(log_likelihood_for_combinations)

            # Some count answers are padded with label -1, we should mask them
            # Shape: (batch_size, # of count answers)
            count_mask = (answer_as_counts != -1).long()
            # Shape: (batch_size, # of count answers)
            clamped_counts = answer_as_counts * count_mask
            log_likelihood_for_counts = torch.gather(count_number_log_probs, 1, clamped_counts)
            # For those padded spans, we set their log probabilities to be very small negative value
            log_likelihood_for_counts = \
                util.replace_masked_values(log_likelihood_for_counts, count_mask.float(), -1e10)
            # Shape: (batch_size, )
            log_marginal_likelihood_for_count = \
                answer_type_log_probs[:, 3] + util.logsumexp(log_likelihood_for_counts)
        ...
        if metadata is not None:
            ...
            for i in range(batch_size):
                ...
                elif answer_type == 2:  # plus_minus combination answer
